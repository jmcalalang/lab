...
resource "azurerm_public_ip" "ce-management" {
  name                = "pip-${random_id.ce-random-id[0].result}-${count.index}"
  ...
  count               = sum([var.ce-instance-count])
  domain_name_label   = "ce-${random_id.ce-random-id[0].result}-${count.index}"
  tags = {
    environment = var.label-environment
    resource    = var.label-resource-type
    Owner       = var.label-email
...
resource "azurerm_network_interface" "nic-sli" {
  name                           = "nic-sli-${random_id.ce-random-id[0].result}-${count.index}"
  location                       = azurerm_resource_group.f5-xc-resource-group.location
  resource_group_name            = azurerm_resource_group.f5-xc-resource-group.name
  count                          = sum([var.ce-instance-count])
...
resource "azurerm_network_interface" "nic-slo" {
  name                           = "nic-slo-${random_id.ce-random-id[0].result}-${count.index}"
  location                       = azurerm_resource_group.f5-xc-resource-group.location
  resource_group_name            = azurerm_resource_group.f5-xc-resource-group.name
  count                          = sum([var.ce-instance-count])
  ip_forwarding_enabled          = false
...
resource "volterra_securemesh_site_v2" "azure-site" {
  name                    = "azure-site-${random_id.ce-random-id[0].result}-${count.index}"
  namespace               = "system"
...
resource "volterra_token" "smsv2_token" {
  name      = "smsv2-token-${random_id.ce-random-id[0].result}-${count.index}"
  namespace = "system"
  type      = 1
...
resource "azurerm_linux_virtual_machine" "ce-instance" {
  name                = "ce-${random_id.ce-random-id[0].result}-${count.index}"
  location            = azurerm_resource_group.f5-xc-resource-group.location
  resource_group_name = azurerm_resource_group.f5-xc-resource-group.name
  ...
  computer_name                   = "ce-${random_id.ce-random-id[0].result}-${count.index}"
...
resource "azurerm_availability_set" "ce-instance" {
  name                = "aset-${random_id.ce-random-id[0].result}"
  location            = azurerm_resource_group.f5-xc-resource-group.location
  resource_group_name = azurerm_resource_group.f5-xc-resource-group.name
  tags = {
    environment = var.label-environment
