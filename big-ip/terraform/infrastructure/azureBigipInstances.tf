...
resource "azurerm_linux_virtual_machine" "big-ip-instance" {
  ...
  admin_ssh_key {
    username   = "azureuser"
    public_key = trimsuffix(tls_private_key.big-ip-ssh-key.public_key_openssh, "\n")
  }
  boot_diagnostics {
  }
  plan {
...
## Shutdown Schedule
resource "azurerm_dev_test_global_vm_shutdown_schedule" "big-ip-instance-group-azure-instances" {
